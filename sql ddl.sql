...
CREATE TABLE `degree` (
  ...
  `degree_description` varchar(45) DEFAULT NULL,
...
CREATE TABLE `scholarship` (
  ...
  `degree_allowed` int(11) DEFAULT NULL,
  PRIMARY KEY (`anncmnt_id`),
  KEY `degreeId_idx` (`degree_allowed`),
  CONSTRAINT `degreeId` FOREIGN KEY (`degree_allowed`) REFERENCES `degree` (`iddegree`) ON DELETE CASCADE ON UPDATE CASCADE,
...
CREATE TABLE `user` (
  ...
  PRIMARY KEY (`email`),
  KEY `role_idx` (`role`),
  CONSTRAINT `roleId` FOREIGN KEY (`role`) REFERENCES `role` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
CREATE TABLE `userschoralshipattributes` (
  ...
  PRIMARY KEY (`user_email`),
  KEY `degreeAnnId_idx` (`degree`),
  CONSTRAINT `degreeAnnId` FOREIGN KEY (`degree`) REFERENCES `degree` (`iddegree`) ON DELETE CASCADE ON UPDATE CASCADE,
